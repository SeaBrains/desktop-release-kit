...
export function computeNextReleaseVersion(currentVersion, mode) {
  ...
  if (mode === "rc-patch" || mode === "rc-minor" || mode === "rc-major") {
    if (parsed.isPrerelease && mode !== "rc-major") {
      throw new Error(
  ...
  if (mode === "rc-next") {
    if (!parsed.isReleaseCandidate || parsed.rcNumber === null) {
      throw new Error(
        `Cannot advance RC number from ${currentVersion}. Expected a version like 2.1.0-rc.1.`,
      );
    }
    return formatReleaseVersion({
      major: parsed.major,
  ...
  if (mode === "promote") {
    if (!parsed.isReleaseCandidate) {
      throw new Error(
        `Cannot promote ${currentVersion}. Expected a release candidate version like 2.1.0-rc.1.`,
      );
    }
